...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            now_ant = place.ant
            if now_ant.is_container and now_ant.can_contain(self):
                now_ant.store_ant(self)
            elif self.is_container and self.can_contain(now_ant):
                self.store_ant(now_ant)
                place.ant = self
            else:
    ...
    def nearest_bee(self):
        ...
        while True:
            if now_place.is_hive:
                return None
            if len(now_place.bees) == 0 or dis > self.upper_bound or dis < self.lower_bound:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        def damage_bees(damage, now_place):
            if now_place == None or len(now_place.bees) == 0:
                return
            bees_goingtodie = []
            for beei in now_place.bees:
                if beei.health <= damage:
                    bees_goingtodie.append(beei)
            for die_bee in bees_goingtodie:
                die_bee.reduce_health(damage)
            for target in now_place.bees:
                target.reduce_health(damage)
        now_place = self.place   
        Ant.reduce_health(self, amount)
        damage_bees(amount, now_place)
        if self.health <= 0:
            damage_bees(self.damage, now_place)    
        # END Problem 5
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        ...
        if self.place is not None:
            damage_bees(self.damage, self.place)
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        now_place = self.place
        while now_place is not None:
            now_place = now_place.exit
            if now_place is not None and now_place.ant is not None:
